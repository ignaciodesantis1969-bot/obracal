...
import { Building2, Clock, Package, ShieldCheck, FileText, ExternalLink, Trash2, Printer } from 'lucide-react';
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-300 shadow-sm p-6 space-y-6">
        ...
        <div className="space-y-6 pt-2">
          ...
          {!certificadoPresupuestoObj ? (
            <div className="p-16 text-center text-slate-400 text-xs border-2 border-dashed border-slate-200 rounded-2xl">
              Seleccione un presupuesto aprobado para desplegar el Certificado de Avance de Obra.
            </div>
          ) : (
                    ...
                    {certificadosDelPresupuestoActual.map((cert) => {
                      ...
                      return (
                        <tr key={certKeyId} className="hover:bg-slate-50">
                          ...
                          <td className="px-4 py-3 text-right font-black">$ {Number(cert?.totalGeneral || cert?.total_general || 0).toLocaleString('es-AR', { maximumFractionDigits: 0 })}</td>
                          <td className="px-4 py-3 text-center flex items-center justify-center gap-2">
                            {pdfLink && <a href={pdfLink} target="_blank" rel="noopener noreferrer" className="px-3 py-1 bg-amber-500 text-slate-950 font-bold rounded-lg text-[10px]"><ExternalLink className="w-3 h-3 inline" /> Ver PDF</a>}
                            <button onClick={() => eliminarCertificadoServidor(certKeyId)} className="p-1 bg-rose-600 text-white rounded-lg"><Trash2 className="w-3.5 h-3.5" /></button>
                          </td>
                        </tr>
                      );
                    })}
              ...
              <tbody className="divide-y">
                {allReportesSice.map((parte, idx) => (
                  <tr key={idx} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-center font-black text-emerald-700">{parte?.totalHorasSuma || parte?.total_horas_suma} hs</td>
      ...
      {tipoCertificadoSubTab === 'compra_materiales' && (
        <div className="space-y-4 pt-2">
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
            <h4 className="text-xs font-black text-slate-900 uppercase">Certificado de Compra de Materiales</h4>
              ...
              <tbody className="divide-y">
                {facturas.map((fac, idx) => (
                  <tr key={idx} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-right font-black">$ {Number(fac?.total || fac?.subtotal || 0).toLocaleString('es-AR', { maximumFractionDigits: 0 })}</td>
                  </tr>
                ))}
              </tbody>
            </table>

...
export default function Usuarios() {
  ...
  // PEGÁ TU NUEVA URL DE GOOGLE AQUÍ ENTRE LAS COMILLAS
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-slate-500 text-sm">Conectado a Google Sheets.</p>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        ...
        <form onSubmit={handleCrear} className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <button 
            type="submit" 
            className="bg-amber-500 hover:bg-amber-600 text-white font-medium rounded-lg px-4 py-2 transition-colors"
          >
            Agregar Usuario
          </button>
        </form>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center">
          ...
          <button onClick={cargarUsuarios} className="text-xs text-amber-600 hover:underline font-medium">Actualizar</button>
        </div>
        ...
        <div className="divide-y divide-slate-100">
          {isLoading ? (
            <div className="p-8 text-center text-slate-500 flex items-center justify-center gap-2">
              <Loader2 className="w-5 h-5 animate-spin text-amber-500" /> Leyendo hoja de cálculo...
            </div>
          ) : usuarios.length > 0 ? (
              ...
              <div key={u.id || index} className="p-4 flex items-center justify-between hover:bg-slate-50">
                ...
                <div className="flex items-center gap-6">
                  <span className="px-2.5 py-1 text-xs font-medium rounded-full bg-slate-100 text-slate-700 flex items-center gap-1">
                    <Shield className="w-3 h-3" /> {String(u.role || 'usuario').toUpperCase()}
                  </span>
                  <button onClick={() => handleDarDeBaja(u.id, u.nombre)} className="text-xs text-red-500 hover:text-red-700 bg-red-50 px-3 py-1.5 rounded-lg flex items-center gap-1">
            ...
            <div className="p-8 text-center text-slate-500 text-sm">No hay usuarios en la hoja de cálculo.</div>
          )}
        </div>
      </div>
    </div>
